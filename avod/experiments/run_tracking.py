...
def test(model_config, eval_config,
    ...
    with tf.Graph().as_default():
        ...
        data_dir = '/media/peiliang/Data/dataset/kitti/raw_data/2011_10_03/raw_tracking_training_image_02_0020'
        for image_idx in range(0, 100000):
            IMAGE_PATH = os.path.join(data_dir, 'image_02/data/{:010d}.png'.format(image_idx))
            LIDAR_PATH = os.path.join(data_dir, 'velodyne_points/data/{:010d}.bin'.format(image_idx))
            CALIB_DIR = '/media/peiliang/Data/dataset/kitti/raw_data/2011_10_03'
            # Keep track of feed_dict speed
            start_time = time.time()
            feed_dict = model._rpn_model.create_single_feed_dict(IMAGE_PATH, LIDAR_PATH, CALIB_DIR)
            ...
            if len(prediction_boxes_3d) > 0:
                ...
                pred_fig, pred_2d_axes, pred_3d_axes = \
                    vis_utils.visualization_from_file(IMAGE_PATH,
                                            display=False,
                                            fig_size=fig_size)
                ...
                # Get calibration
                point_cloud = kitti_utils.get_point_cloud_from_file('lidar',
                                                               stereo_calib,
                                                               LIDAR_PATH,
                                                               (370, 1242))
